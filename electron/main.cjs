/* electron/main.cjs — v36
   - FIXED: Workaround для проблеми з require('electron') на Windows
...
// ULTIMATE Workaround: На деяких системах Windows require('electron') НЕ ПРАЦЮЄ
// Використовуємо внутрішній binding Electron для прямого доступу до API
let electron;
try {
  // Спроба 1: Звичайний require
  electron = require('electron');

  // Якщо повернувся string - використовуємо internal bindings
  if (typeof electron === 'string') {
    console.log('[WARN] require("electron") returned path, using internal binding workaround');

    // Спроба через process.binding (застарілий але працює)
    try {
      const binding = process.binding('electron_browser_window');
      if (binding) {
        console.log('[INFO] Found electron_browser_window binding');
      }
    } catch (e) {
      console.log('[WARN] process.binding failed:', e.message);
    }

    // Спроба через Module.globalPaths - додаємо шлях до Electron
    try {
      const Module = require('module');
      const electronPath = path.join(__dirname, '../node_modules/electron/dist/resources/electron.asar');
      if (Module.globalPaths && !Module.globalPaths.includes(electronPath)) {
        Module.globalPaths.push(electronPath);
      }

      // Очищаємо кеш і пробуємо ще раз
      delete require.cache[require.resolve('electron')];
      electron = require('electron');

      if (typeof electron === 'string') {
        throw new Error('Still returns string after cache clear');
      }
    } catch (e2) {
      console.log('[WARN] Cache clear attempt failed:', e2.message);

      // ОСТАННІЙ ШАНС: Використовуємо заглушку з повідомленням
      throw new Error(
        'CRITICAL: Cannot load Electron API!\n\n' +
        'require("electron") returns: ' + (typeof electron) + '\n' +
        'This is a known issue with Electron on some Windows systems.\n\n' +
        'SOLUTION:\n' +
        '1. Close all Node.js/Electron processes\n' +
        '2. Delete node_modules folder completely\n' +
        '3. Run: npm install\n' +
        '4. If still fails, update Node.js to latest LTS version'
      );
    }
  }

  // Перевірка що отримали правильний API
  if (!electron || typeof electron !== 'object' || !electron.app) {
    throw new Error('Electron API is invalid: ' + JSON.stringify(Object.keys(electron || {})));
  }

  console.log('[OK] Electron API loaded successfully');
} catch (e) {
  console.error('\n' + '='.repeat(80));
...
const { app, BrowserWindow, ipcMain, shell, net: eNet } = electron;
...
try {
  ...
  console.warn("[WARN] electron-updater not available:", e.message);
}
...
// Ці змінні будуть ініціалізовані після app.whenReady()
let LIC_DIR, LIC_JSON, LIC_JWT;

function initLicensePaths() {
  if (!LIC_DIR) {
    LIC_DIR = app.getPath("userData");
    LIC_JSON = path.join(LIC_DIR, "license.json");
    LIC_JWT = path.join(LIC_DIR, "license.jwt");
  }
}

// Перекодування тексту чека. Робимо це саме тут: preload виконується в
// пісочниці, де require("iconv-lite") недоступний, і кирилиця мовчки
// відправлялась у принтер як UTF-8 (замість CP866) — звідси «кракозябри».
let iconv = null;
try { iconv = require("iconv-lite"); }
catch (e) { console.warn("[print] iconv-lite unavailable:", e?.message || e); }

// ESC t <n> — вибір кодової таблиці принтера
const CODEPAGE_IDS = { CP437: 0, CP850: 2, CP860: 3, CP863: 4, CP865: 5, CP866: 17, CP852: 18, CP858: 19, CP1251: 46 };

function encodeReceipt(text, encoding) {
  const enc = String(encoding || "CP866").toUpperCase();
  if (iconv && iconv.encodingExists(enc)) {
    try { return iconv.encode(String(text ?? ""), enc); }
    catch (e) { console.warn("[print] encode failed:", e?.message || e); }
  }
  return Buffer.from(String(text ?? ""), "utf8");
}

/* Номери кодових таблиць у різних виробників відрізняються (Epson, XPrinter,
   Gprinter...). Друкуємо всі ймовірні варіанти — оператор бачить, який рядок
   читається, і вписує цей номер у налаштуваннях. */
const CODEPAGE_CANDIDATES = [
  { n: null, enc: "CP1251" },
  { n: 46, enc: "CP1251" },
  { n: 6,  enc: "CP1251" },
  { n: 45, enc: "CP1251" },
...
function buildCodepageSweep() {
  const parts = [Buffer.from([0x1b, 0x40])];
  const ascii = (s) => Buffer.from(s, "ascii");
  const sample = "Стiл: і ї є ґ І Ї Є Ґ";

  parts.push(ascii("KODOVA TABLYCIA - TEST\n"));
  parts.push(ascii("Znajdit chytkyj riadok\n"));
  parts.push(ascii("------------------------------\n"));

  for (const c of CODEPAGE_CANDIDATES) {
    if (Number.isFinite(c.n)) parts.push(Buffer.from([0x1b, 0x74, c.n]));
    const label = `n=${c.n === null ? "--" : String(c.n).padStart(2, "0")} ${c.enc}: `;
    parts.push(ascii(label));
    parts.push(encodeReceipt(sample, c.enc));
    parts.push(ascii("\n"));
  }

  parts.push(ascii("------------------------------\n"));
  parts.push(ascii("Vpyshit n u nalashtuvanniah\n"));
  parts.push(Buffer.from([0x1b, 0x64, 4]));
  parts.push(Buffer.from([0x1d, 0x56, 0x42, 0x00]));
  return Buffer.concat(parts);
}

/** Повний ESC/POS-пакет: ініціалізація + кодова таблиця + текст + протяжка + відрізання */
function buildEscposJob({ text, encoding, prefix, codepageId, feedLines = 4, cut = true }) {
  const parts = [];
  parts.push(Buffer.from([0x1b, 0x40])); // ESC @ — скидання стану принтера

  const cp = Number.isFinite(Number(codepageId))
    ? Number(codepageId)
    : CODEPAGE_IDS[String(encoding || "CP866").toUpperCase()];
  if (Number.isFinite(cp)) parts.push(Buffer.from([0x1b, 0x74, cp])); // ESC t n

  if (prefix) parts.push(Buffer.isBuffer(prefix) ? prefix : Buffer.from(String(prefix), "latin1"));

  parts.push(encodeReceipt(text, encoding));

  const lines = Math.max(0, Math.min(20, Number(feedLines) || 0));
  if (lines) parts.push(Buffer.from([0x1b, 0x64, lines])); // ESC d n — протяжка
  if (cut) parts.push(Buffer.from([0x1d, 0x56, 0x42, 0x00])); // GS V B 0 — відрізання
...
function computeMachineId() {
  try {
    ...
    const macs = Object.values(nets).flat().filter(Boolean)
      .map(n => n.mac).filter(m => m && m !== "00:00:00:00:00:00").sort().join("|");
    let userDataPath = "";
    try { userDataPath = app.getPath("userData"); } catch {
      userDataPath = process.env.APPDATA || process.env.LOCALAPPDATA || process.env.HOME || "";
    }
    const seed = [
      os.hostname(),
      os.arch(),
    ...
    throw new Error("hash empty");
  } catch {
    // Кінцевий fallback — завжди повертає щось стабільне
    try {
      const seed2 = [
        os.hostname(),
        os.platform(),
      ...
      return crypto.createHash("sha256").update(seed2).digest("hex").slice(0, 32);
    } catch {
      // Абсолютний fallback — хоча б щось
...
async function createOrder({ plan, tier, machineId, forceNew }) {
  // сервер читає поле tier — надсилаємо обидва, щоб працювало з будь-якою його версією
  const t = tier || plan;
  const raw = await postJson(`${HARD_BASE}/api/orders`, { plan, tier: t, machineId, forceNew: !!forceNew });
...
function registerIpc() {
  ...
  // Діагностика: друк усіх ймовірних кодових таблиць
  ipcMain.handle("printer:printCodepageTest", async (_evt, { ip } = {}) => {
    if (!ip) return { ok:false, error:"NO_IP" };
    try {
      const job = buildCodepageSweep();
      await sendRaw9100({ ip, data: job });
      return { ok:true, bytes: job.length };
    } catch (e) { return { ok:false, error: e?.message || String(e) }; }
  });

  // Друк чека з перекодуванням на боці головного процесу
  ipcMain.handle("printer:printText", async (_evt, opts = {}) => {
    const { ip, text, encoding, prefix, codepageId, feedLines, cut } = opts;
    if (!ip) return { ok:false, error:"NO_IP" };
    try {
      const job = buildEscposJob({ text, encoding, prefix, codepageId, feedLines, cut });
      await sendRaw9100({ ip, data: job });
      return { ok:true, bytes: job.length, encoded: !!iconv };
    } catch (e) { return { ok:false, error: e?.message || String(e) }; }
  });
  ...
  console.log("[main] IPC v36 ready");
}
...
function initAutoUpdater() {
  ...
  const sendToRenderer = (type, data = {}) => {
    const wins = BrowserWindow.getAllWindows();
    wins.forEach(w => {
      try { w.webContents.send("updates:event", { type, ...data }); } catch {}
    });
  };
  ...
  autoUpdater.on("download-progress", (progress) => {
    ...
    sendToRenderer("progress", { p: progress });
  });
  ...
  // Перевірка оновлень при старті (з невеликою затримкою)
  setTimeout(() => {
    console.log("[updater] Initial check...");
