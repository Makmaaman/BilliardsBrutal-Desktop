 ...
 */
export function validateReservation(newRes, existingList) {
  const errors = [];
  ...
  if (!errors.length) {
    for (const r of existingList || []) {
      // редагування: пропускаємо саме себе (id можуть бути рядком або числом)
      if (newRes.id && String(r.id) === String(newRes.id)) continue;
      if (Number(r.tableId) === Number(tableId) && r.status !== "cancelled") {
