...
export default function ActivationScreen({ onActivated }) {
  ...
  /* Поки є неоплачений рахунок — перевіряємо оплату кожні 5 с, щоб ліцензія
     застосувалась сама, без ручного натискання «Я оплатив». */
  useEffect(() => {
    stopPolling();
    if (!order?.id || !machineId) return;
    pollRef.current = setInterval(() => { verify({ silent: true }); }, 5000);
    return stopPolling;
  }, [order?.id, machineId]);
  ...
  const verify = async ({ silent = false } = {}) => {
    if (!order?.id) { if (!silent) setMsg("Немає рахунку для перевірки."); return false; }
    if (!silent) { setMsg(""); setBusy(true); }
    try {
      const act = await window.license?.activate?.(machineId, order.id);
      if (!act?.ok) {
        const err = act?.error || "";
        // оплата ще не пройшла — під час автоперевірки просто чекаємо далі
        if (err.includes("ORDER_NOT_PAID")) {
          if (!silent) setMsg("Оплата ще не надійшла. Перевірка триває автоматично.");
          return false;
        }
        if (err.includes("ORDER_NOT_FOUND")) {
          // можливо, оплата була раніше — пробуємо відновити за Machine ID
          const restored = await restoreLicense({ silent: true });
          if (restored) return true;
          if (!silent) { await clearOrder(); setMsg("Рахунок не знайдено. Створіть новий."); }
          return false;
        }
        if (silent) return false;
        throw new Error(err || "ACTIVATE_FAILED");
      }
      if (act.license) {
        ...
        return true;
      } else if (!silent) { setMsg(`Статус: ${act.status || "очікування..."}`); }
      return false;
    } catch (e) { if (!silent) setMsg("Помилка: " + (e.message || String(e))); return false; }
    finally { if (!silent) setBusy(false); }
  };

  /** Відновлення ліцензії за Machine ID (оплата вже пройшла, але рахунок втрачено) */
  const restoreLicense = async ({ silent = false } = {}) => {
    if (!machineId) return false;
    if (!silent) { setMsg(""); setBusy(true); }
    try {
      const base = (window.__LICENSE_API_BASE__ || "https://billiardsbrutal-desktop-1.onrender.com/").replace(/\/+$/, "/");
      const res = await fetch(base + "api/license/restore", {
        method: "POST",
      ...
      if (data?.ok && data.license) {
        const planMeta = PLANS.find(p => p.id === (data.tier || plan)) || { limit: 10, mode: "full" };
        const saved = await window.license?.applyJwt?.(data.license, {
          plan: data.tier || plan, mode: planMeta.mode, tablesLimit: planMeta.limit,
        });
        if (!saved?.ok) throw new Error(saved?.error || "Не вдалося застосувати ліцензію (applyJwt).");
        stopPolling();
        setMsg("Ліцензію відновлено ✅ Відкриваю головний екран...");
        try { await window.license?.activated?.(); } catch {}
        setTimeout(() => { try { window.location.reload(); } catch {} }, 300);
        try { await onActivated?.(); } catch {}
        return true;
              ...
              <div className="flex flex-wrap gap-2">
                {!order ? (
                  <>
                    ...
                    <button className="h-10 px-4 rounded-lg bg-white text-slate-700 ring-1 ring-slate-300 hover:bg-slate-50 whitespace-nowrap" disabled={busy || !machineId} onClick={() => restoreLicense()} title="Якщо оплата вже пройшла на цьому ПК">
                      Відновити ліцензію
                    </button>
                  </>
                ) : (
