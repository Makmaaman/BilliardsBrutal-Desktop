...
function BalancePill({ value }){
  const color = value > 0
    ...
    : value < 0
      ...
      : "bg-slate-800/60 text-slate-200 ring-slate-600/40";
...
function SegBtn({ active, children, onClick, tone="slate" }){
  const on = active ? `bg-${tone}-600 text-white ring-${tone}-500` : "bg-slate-900/60 text-emerald-100 ring-emerald-500/30 hover:bg-slate-800/60";
...
function Drawer({ open, onClose, children }){
  ...
  return (
    <div className={cx(
      ...
      <div className="h-full w-full bg-gradient-to-br from-slate-900/98 via-emerald-950/98 to-slate-950/98 text-emerald-50 md:rounded-l-2xl shadow-2xl ring-1 ring-emerald-500/30 border border-emerald-500/30 overflow-y-auto">
        {children}
      </div>
...
export default function CustomersModal({ onClose }){
  ...
  async function handleImportCsv(file){
    ...
    try{
      ...
      if (!await confirmAsync(`Імпортувати ${rows.length} запис(ів)?`)) return;
      for (const r of rows){
        try{
  ...
  async function deleteCustomer(c){
    if (!c?.id) return;
    if (!await confirmAsync(`Видалити клієнта «${c.name||c.phone||c.id}»?`)) return;
    try{
      await api("customers:remove", { id: c.id });
      await load();
  ...
  useLayoutEffect(() => {
    ...
    setBox((prev) => {
      ...
      const w = Math.min(START_W, maxW);
      const h = Math.min(START_H, maxH);
  ...
  const dialogStyle = box
    ? { left: box.x, top: box.y, width: box.w, height: box.h, transform: "none" }
    : { left: "50%", top: "50%", transform: "translate(-50%, -50%)" };

  return (
    <div className="fixed inset-0 z-[12000]">
      <div className="absolute inset-0 bg-slate-950/80 backdrop-blur-md" onMouseDown={onClose} />

      {/* трішки вища модалка */}
      <div
        ref={dialogRef}
        style={dialogStyle}
        className="absolute w-[min(900px,92vw)] bg-gradient-to-br from-slate-900/95 via-emerald-950/95 to-slate-950/95 text-emerald-50 rounded-2xl shadow-2xl ring-1 ring-emerald-500/30 border border-emerald-500/30 overflow-hidden h-[min(640px,82vh)]"
        role="dialog" aria-modal="true"
        onMouseDown={(e)=>e.stopPropagation()}
      >
        {/* Header */}
        <div className="px-5 py-3 border-b border-emerald-500/20 bg-gradient-to-r from-emerald-900/60 via-emerald-800/50 to-slate-900/60 backdrop-blur flex items-center justify-between cursor-move select-none" onPointerDown={startDrag}>
          <div className="flex items-center gap-3">
            <div className="text-lg font-semibold text-emerald-50">Клієнти</div>
            <span className="text-xs px-2 py-0.5 rounded-full bg-emerald-900/40 text-emerald-100 ring-1 ring-emerald-500/30">
              усього: {toUA(list.length)}
            </span>
            {err && <span className="text-xs text-rose-300">• {err}</span>}
          </div>
          <div className="flex items-center gap-2">
            <button className="h-9 px-3 rounded-xl border border-emerald-400/40 bg-slate-900/50 text-emerald-100 hover:bg-emerald-900/40 text-sm flex items-center gap-1 outline-none focus-visible:ring-2 focus-visible:ring-emerald-500/50" onClick={openNew}>
              <Icon.Plus/> Додати
            </button>

            <label className="h-9 px-3 rounded-xl border border-emerald-400/40 bg-slate-900/50 text-emerald-100 hover:bg-emerald-900/40 text-sm flex items-center gap-1 cursor-pointer outline-none focus-within:ring-2 focus-within:ring-emerald-500/50">
              <input type="file" accept=".csv,text/csv" className="hidden" onChange={(e)=>handleImportCsv(e.target.files?.[0])}/>
              <Icon.Upload/> Імпорт CSV
            </label>

            <button className="h-9 px-3 rounded-xl border border-emerald-400/40 bg-slate-900/50 text-emerald-100 hover:bg-emerald-900/40 text-sm flex items-center gap-1 outline-none focus-visible:ring-2 focus-visible:ring-emerald-500/50" onClick={handleExportCsv}>
              <Icon.Download/> Експорт CSV
            </button>

            <button className="h-9 px-3 rounded-xl bg-slate-800 text-emerald-100 hover:bg-slate-700 text-sm outline-none focus-visible:ring-2 focus-visible:ring-emerald-500/50" onClick={onClose}>Закрити</button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-[320px,1fr]">
          {/* Left pane */}
          <div className="border-r border-emerald-500/20 bg-slate-900/30 p-4 space-y-4">
            <div>
              <label className="text-xs text-emerald-200/70">Пошук</label>
              <div className="relative mt-1">
                <input
                  ref={searchRef}
                  className="w-full h-10 pl-9 pr-3 rounded-xl ring-1 ring-emerald-500/30 bg-slate-900/60 text-emerald-50 placeholder:text-slate-400 focus:ring-2 focus:ring-emerald-500/50 outline-none"
                  placeholder="Ім’я, телефон, ID…"
                  value={q}
                  onChange={(e)=>{ setQ(e.target.value); setPage(1); }}
                />
                <svg className="absolute left-3 top-1/2 -translate-y-1/2 opacity-70 pointer-events-none text-emerald-200/70" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="text-xs text-emerald-200/70">Сортування</label>
                <select className="mt-1 w-full h-10 px-2 rounded-xl ring-1 ring-emerald-500/30 bg-slate-900/60 text-emerald-50 outline-none focus:ring-2 focus:ring-emerald-500/50" value={sortBy} onChange={(e)=>setSortBy(e.target.value)}>
              ...
              <div>
                <label className="text-xs text-emerald-200/70">Порядок</label>
                <select className="mt-1 w-full h-10 px-2 rounded-xl ring-1 ring-emerald-500/30 bg-slate-900/60 text-emerald-50 outline-none focus:ring-2 focus:ring-emerald-500/50" value={sortDir} onChange={(e)=>setSortDir(e.target.value)}>
            ...
            <div>
              <label className="text-xs text-emerald-200/70">Баланс</label>
            ...
            <div className="text-xs text-emerald-200/70 pt-2">Знайдено: <b>{toUA(filtered.length)}</b></div>
          </div>

          {/* Right pane – list */}
          <div className="p-4">
            <div className="rounded-2xl ring-1 ring-emerald-500/30 overflow-hidden bg-slate-900/40 relative">
              <table className="w-full table-fixed">
                ...
                <thead className="bg-slate-800/60 sticky top-0 z-[5]">
                  <tr className="text-left text-emerald-200/80">
                ...
                <tbody>
                  {loading && (
                    <tr><td colSpan={3} className="px-4 py-8 text-center text-emerald-200/60">Завантаження…</td></tr>
                  )}

                  {!loading && pageItems.length===0 && (
                    <tr><td colSpan={3} className="px-4 py-8 text-center text-emerald-200/60">Нічого не знайдено</td></tr>
                  )}

                  {!loading && pageItems.map(c => (
                    <tr key={c.id} className="even:bg-emerald-900/20">
                      <td className="px-4 py-3 align-middle">
                        <div className="flex items-start gap-3 min-w-0">
                          <div className="mt-0.5 w-9 h-9 rounded-full bg-gradient-to-br from-emerald-500/20 to-sky-500/20 ring-1 ring-emerald-400/30 grid place-items-center text-[11px] text-emerald-100 shrink-0">
                            {initials(c.name || c.phone || c.id)}
                          </div>
                          <div className="min-w-0">
                            <div className="font-medium truncate">{c.name || "—"}</div>
                            <div className="text-[11px] text-emerald-200/60 truncate">ID: {c.id}</div>
                            {/* БОНУСИ ПЕРЕНЕСЕНО СЮДИ */}
                            <div className="mt-1">
                              <BalancePill value={Number(c.bonusBalance||0)} />
                              <span className="ml-2 text-[11px] text-emerald-200/60">1 бонус = 1 грн</span>
                            </div>
                          </div>
                        </div>
                      </td>
                      ...
                      <td className="px-4 py-3 align-middle">
                        {c.phone ? <a className="text-emerald-100 hover:underline whitespace-nowrap" href={`tel:${c.phone}`}>{phonePretty(c.phone)}</a> : <span className="text-emerald-200/50">—</span>}
                      </td>

                      <td className="px-4 py-3 align-middle">
                        <div className="flex justify-end gap-2 whitespace-nowrap">
                          <button
                            className="h-8 px-2 rounded-lg border border-emerald-400/40 text-emerald-100 hover:bg-emerald-900/40 flex items-center gap-1 outline-none focus-visible:ring-2 focus-visible:ring-emerald-500/50"
                            onClick={()=>openEdit(c)}
                          >
                            <Icon.Pencil/><span className="hidden sm:inline">Редагувати</span>
                          </button>
                          <button
                            className="h-8 px-2 rounded-lg border border-rose-400/40 text-rose-100 hover:bg-rose-900/30 flex items-center gap-1 outline-none focus-visible:ring-2 focus-visible:ring-rose-500/40"
            ...
            {totalPages > 1 && (
              <div className="mt-3 flex items-center justify-between text-sm">
                <div className="text-emerald-200/70">Сторінка {page} з {totalPages}</div>
                <div className="flex items-center gap-2">
                  <button className="h-8 px-3 rounded-xl border border-emerald-400/40 text-emerald-100 hover:bg-emerald-900/40 disabled:opacity-50 outline-none focus-visible:ring-2 focus-visible:ring-emerald-500/50"
        ...
        <Drawer open={drawer.open} onClose={closeDrawer}>
          <div className="relative">
            <div className="sticky top-0 z-10 px-4 py-3 border-b border-emerald-500/20 bg-gradient-to-r from-emerald-900/60 via-emerald-800/50 to-slate-900/60 backdrop-blur flex items-center justify-between">
              <div className="text-sm font-semibold text-emerald-50">
                {drawer.customer?.id ? "Редагувати клієнта" : "Новий клієнт"}
              </div>
              <div className="flex items-center gap-2">
                <button className="h-9 px-3 rounded-lg bg-emerald-600 text-white hover:bg-emerald-500 disabled:opacity-50 outline-none focus-visible:ring-2 focus-visible:ring-emerald-500/50"
                        disabled={drawer.saving}
                        onClick={saveCustomer}>
                  {drawer.saving ? "Збереження…" : "Зберегти"}
                </button>
                <button className="h-9 w-9 grid place-items-center rounded-lg border border-emerald-400/40 text-emerald-100 hover:bg-emerald-900/40 outline-none focus-visible:ring-2 focus-visible:ring-emerald-500/50"
            ...
            <form className="p-4 space-y-5" onSubmit={(e)=>{ e.preventDefault(); saveCustomer(); }}>
              <div className="flex items-center gap-3">
                <div className="w-11 h-11 rounded-full bg-gradient-to-br from-emerald-500/20 to-sky-500/20 ring-1 ring-emerald-400/30 grid place-items-center text-xs text-emerald-100">
                  {initials(drawer.customer?.name || drawer.customer?.phone || drawer.customer?.id)}
                </div>
                {!!drawer.customer?.id && (
                  <div className="text-[11px] text-emerald-200/60">ID: {drawer.customer.id}</div>
                )}
              </div>

              <div className="space-y-2">
                <label className="text-xs text-emerald-200/70">Ім’я</label>
                <input
                  className="w-full h-10 px-3 rounded-xl ring-1 ring-emerald-500/30 bg-slate-900/60 text-emerald-50 placeholder:text-slate-400 focus:ring-2 focus:ring-emerald-500/50 outline-none"
              ...
              <div className="space-y-2">
                <label className="text-xs text-emerald-200/70">Телефон</label>
                <input
                  className="w-full h-10 px-3 rounded-xl ring-1 ring-emerald-500/30 bg-slate-900/60 text-emerald-50 placeholder:text-slate-400 focus:ring-2 focus:ring-emerald-500/50 outline-none"
              ...
              <div className="space-y-2">
                <label className="text-xs text-emerald-200/70">Бонусний баланс</label>
                <div className="flex items-center gap-2">
                  <BalancePill value={Number(drawer.customer?.bonusBalance||0)} />
                <span className="text-xs text-emerald-200/60">1 бонус = 1 грн</span>
                </div>
              </div>

              <div className="space-y-2">
                <label className="text-xs text-emerald-200/70">Нарахувати / Списати</label>
                <div className="flex flex-wrap items-center gap-2">
                  <input
                    type="number"
                    className="w-36 h-10 px-2 rounded-xl ring-1 ring-emerald-500/30 bg-slate-900/60 text-emerald-50 placeholder:text-slate-400 focus:ring-2 focus:ring-emerald-500/50 outline-none"
