...
export default function RentalsModal({ onClose, cues = [], setCues = () => {} }) {
  ...
  return (
    <ModalShell
      title="Оренда — киї"
      onClose={onClose}
      containerStyle={{ width: "960px", maxWidth: "92vw", height: "700px", maxHeight: "86vh" }}
      footer={
        <div className="flex justify-between w-full">
          ...
          <div className="flex gap-2">
            <button className="h-9 px-4 rounded-lg border border-slate-600 text-emerald-100 hover:bg-slate-700/60" onClick={onClose}>Відмінити</button>
            <button className="h-9 px-4 rounded-lg bg-emerald-600 text-white hover:bg-emerald-500" onClick={saveAll}>Зберегти</button>
          </div>
        </div>
      }
    >
