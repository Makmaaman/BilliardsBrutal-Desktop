...
export default function TariffsModal({
  ...
  return (
    <ModalShell title="Тарифи" onClose={onClose} containerStyle={{ width: "1100px", maxWidth: "92vw", height: "760px", maxHeight: "86vh" }} footer={
      <div className="flex justify-between items-center">
        <div className="flex gap-2">
          <button className="h-9 px-3 rounded-lg bg-emerald-500/20 text-emerald-200 ring-1 ring-emerald-400/40 hover:bg-emerald-500/30" onClick={applyFlatAllDay}>Цілий день = базова</button>
          <button className="h-9 px-3 rounded-lg bg-emerald-500/20 text-emerald-200 ring-1 ring-emerald-400/40 hover:bg-emerald-500/30" onClick={applyWeekdayWeekendPreset}>Будні/Вечір/Вихідні</button>
        </div>
        <div className="flex gap-2">
          <button className="h-9 px-4 rounded-lg bg-emerald-600 text-white hover:bg-emerald-500" onClick={save}>Зберегти</button>
      ...
      {err && <div className="mt-3 text-sm text-rose-300 bg-rose-500/20 px-3 py-2 rounded-lg">{err}</div>}
      ...
      <div className="mt-4 grid gap-3">
        ...
        {localRules.length === 0 && (
          <div className="text-sm text-emerald-200/80 bg-slate-800/40 px-4 py-3 rounded-xl border border-emerald-500/20">Правил немає. Додайте через «Додати правило» або використайте пресет.</div>
        )}
      </div>
